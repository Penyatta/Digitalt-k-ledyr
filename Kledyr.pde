class Dyr {
  ...
  void tegnDyr() {
    ...
    if (tungeIBrug) {
      ...
      strokeWeight(10);
      strokeCap(ROUND);
      line(mundPosX+cos(mundVinkel+PI/2)*sX/25-camX, mundPosY+sin(mundVinkel+PI/2)*sX/25-camY, tungeX-camX, tungeY-camY);
      stroke(0);
      strokeWeight(3);
      strokeCap(SQUARE);
    }
...
void tunge() {
  if (!flemming.tungeIBrug) {
    ...
    if (flemming.tungeUd) {
      ...
      println(flemming.mundPosX+cos(flemming.mundVinkel+PI/2)*flemming.sX/25);
